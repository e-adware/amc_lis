...
-->
		...
		<?php
			...
			{
				$dept_info=mysqli_fetch_assoc(mysqli_query($link, "SELECT `name` FROM `test_department` WHERE `id`='$tst[type_id]'"));
...
<!--
				<td>
					<input type="text" id="test_code<?php echo $tst['testid']; ?>" value="<?php echo $tst['test_code']; ?>" class="span1 test_code" onkeyup="test_code_change_up('<?php echo $tst['testid']; ?>',event,this.value)">
				</td>
-->
				<td>
...
if($_POST["type"]=="test_code_change")
{
	$val=mysqli_real_escape_string($link, $_POST['val']);
	$id=mysqli_real_escape_string($link, $_POST['id']);
	
	mysqli_query($link, " UPDATE `testmaster` SET `test_code`='$val' WHERE `testid`='$id' ");
	
	$val=mysqli_fetch_array(mysqli_query($link, " SELECT `test_code` FROM `testmaster` WHERE `testid`='$id' "));
