...
<?php
		...
		while($q=mysqli_fetch_array($qrtest))
		{
			...
			$sort_count=mysqli_fetch_array(mysqli_query($link,"SELECT COUNT(`testid`) AS `count` FROM `report_sorting` WHERE `opd_id` = '$q[bill_id]' AND `patient_id` = '$q[patient_id]'"));
			...
			?>
			 <tr>
				...
				<td><?php echo $sort_count['count']; ?></td>
				<td>
...
if ($type == 'sort') {
	...
	$testid_explode = explode("@", $tst);

	$dlt_sort=mysqli_query($link,"DELETE FROM `report_sorting` WHERE `opd_id`='$opd_id' AND `ipd_id`='$ipd_id' AND `patient_id` = '$uhid'");

	foreach ($testid_explode as $testid_doc) {
		if (!empty($testid_doc)) {
			$sql = "INSERT INTO `report_sorting` (`patient_id`, `opd_id`, `ipd_id`, `batch_no`, `testid`, `user`, `date`, `time`) VALUES ('$uhid', '$opd_id', '$ipd_id', '$batch_no', '$testid_doc', '$user', '$date', '$time')";
