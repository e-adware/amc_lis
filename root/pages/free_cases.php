...
<div class="container-fluid">
    <table class="table ">
            ...
            <th>
                <div style="display: flex; justify-content: center; align-items: center; gap: 10px;">
                    ...
                    <input id="to_date" type="text" class="datepicker span2" value="<?php echo date('Y-m-d'); ?>"
                        readonly />
                    <select id="sel_time" onchange="load_data()" name="time_slot" class="span2">
        ...
        <tr>
            <th>
                <div onchange="load_data()"
                    style="display: flex; justify-content: center; align-items: center; gap: 10px;">
                    <select id="free_type" class="span2">
                        <option value="0">Free Type</option>
                        <?php
                        $free_type_qry = mysqli_query($link, "SELECT * FROM `pat_free_type` ORDER BY `name`");
...
<script>
    ...
    function load_data() {
        $("#loader").show();
        $.post("pages/free_cases_data.php",
            {
                type: "load_data",
                to_date: $("#to_date").val(),
    ...
    function exportTableToExcel(fdate, tdate, test, time, freetest) {
        var url = "pages/free_cases_excel.php?from_date=" + btoa(fdate) + "&to_date=" + btoa(tdate) + "&sel_test=" + btoa(test) +
            "&time_period=" + btoa(time) + "&freetest=" + btoa(freetest);
        window.open(url, 'Window', 'scrollbars=1,menubar=1,toolbar=0,height=670,width=1000');

    }

    function printTable(fdate, tdate, test, time, freetest) {
        $.post("pages/free_cases_data.php",
            {
                type: "load_data",
                to_date: tdate,
            ...
            function (data, status) {
                ...
                printWindow.document.write('<html><head><title>Free Cases</title>');
                printWindow.document.write('<style>@media print {#no_print {display: none !important;}} table {width: 100%; border-collapse: collapse;} th, td {border: 2px solid #ccc; padding: 8px;} h4 {margin: 0 0 10px 0;	} .head_space {margin-bottom: 80px;}</style>');
                printWindow.document.write('</head><body>');
                // printWindow.document.write('<h4 style="text-align: center;">Free Cases Data Dated From: ' + fdate + ' To: ' + tdate + '</h4 > ');
