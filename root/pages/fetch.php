	...
	function savePatData()
	{
		...
		else
		{
			...
			$.post("pages/fetch_ajax.php",
			{
				...
				aTestRts	:$("#aTestRts").val().trim(),
				regDate		:$("#regDate").val().trim(),
				testDepts	:$("#testDepts").val().trim(),
				patType		:$("#patType").val().trim(),
				freeType	:$("#freeType").val().trim(),
				authBy		:$("#authBy").val().trim(),
				schemeName	:$("#schemeName").val().trim(),
				discountAmt	:$("#discountAmt").val().trim(),
				pmjay		:$("#pmjay:checked").length,
				jssk		:$("#jssk:checked").length,
				rbsk		:$("#rbsk:checked").length,
				user		:$("#user").text().trim(),
				type		:2
			},
			function(data,status)
