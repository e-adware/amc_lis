...
<tr>
	...
	<?php
		...
		while($p_tst=mysqli_fetch_array($p_test))
		{
			...
			//if($ent["doc"]>0)
			$checked="";
			$sort_chckd= mysqli_fetch_array(mysqli_query($link,"SELECT `slno` FROM `report_sorting` WHERE `patient_id`='$pid' AND `opd_id`='$opd_id' AND `ipd_id`='$ipd_id' AND `batch_no`='$batch' AND `testid`='$p_tst[testid]'"));
			if($sort_chckd)
			{
				$checked="checked";
				//$clss="tst";
			...
			echo "<td><input type='checkbox' value='$p_tst[testid]' name='grp_td$i' id='$p_tst[testid]_tst' class='$clss' onclick='test_print_group(this.value)' $print_disabled $checked><label><span></span></label></td>";
		...
		?>
			<tr>
				<th colspan="9"> 
					...
					<button id="grp_print" class="btn btn-print" onclick="sort_reports()"><i class="icon-print"></i> Sort By </button>
