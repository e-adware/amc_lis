...
?>
	...
	while($w=mysqli_fetch_array($ward))
	{
		...
		while($b=mysqli_fetch_array($beds))
		{
			...
			$room_det=mysqli_query($link,"select * from bed_master where ward_id='$w[ward_id]' and room_id='$b[room_id]' order by bed_id");
			while($rd=mysqli_fetch_array($room_det))
			{
				$style="auto;margin-left:10px;";
				...
				else if($rd["status"]==1)
				{
					...
					//echo "<button class='btn btn-warning' id='$rd[bed_id]' style='margin-right: 5px;'>Bed No: $rd[bed_no]<br>₹".number_format($rd["charges"],2)."</button>";
				...
				else
				{
					...
					{
						//$style.="background-color:#5cb85c";
						echo "<button class='btn btn-search' id='$b[bed_id]' onclick=\"bed_asign('$w[ward_id]','$rd[bed_id]','$w[name]','$rd[bed_no]')\" style='margin-right: 5px;'>Bed No: $rd[bed_no]<br>₹".number_format($rd["charges"],2)."</button>";
					}
				}
...
if($type=="pat_save")
{
	...
	$addoc_id=mysqli_real_escape_string($link, $_POST["addoc_id"]);
	$atdoc_id=mysqli_real_escape_string($link, $_POST["atdoc_id"]);
	$dept_id=mysqli_real_escape_string($link, $_POST["dept_id"]);
	$bed_id=mysqli_real_escape_string($link, $_POST["bed_id"]);
	...
	if(!$addoc_id)
	{
		echo $patient_id."@".$opd_id."@".$emp_access_str."@Select Admitted By";
		exit();
	}
	...
	if($opd_id=="0")
	{
		if(!$bed_id)
		{
			echo $patient_id."@".$opd_id."@".$emp_access_str."@Assign Bed";
			exit();
		}
	}
	...
	else
	{
		if($opd_id=="0")
		{
			...
			if(mysqli_query($link, " INSERT INTO `uhid_and_opdid`(`patient_id`, `opd_id`, `date`, `time`, `user`, `type`, `ipd_serial`, `refbydoctorid`, `center_no`, `hguide_id`, `branch_id`, `urgent`, `remarks`, `doc_discount`) VALUES ('$patient_id',NULL,'$date','$time','$user','$p_type_id','','$refbydoctorid','$sel_center','$hguide_id','$branch_id','$urgent','$remarks','$doc_discount') "))
			{
				...
				if(mysqli_query($link," UPDATE `uhid_and_opdid` SET `opd_id`='$opd_id',`ipd_serial`='$opd_serial' WHERE `slno`='$last_row_num' "))
				{
					...
					mysqli_query($link,"INSERT INTO `ipd_bed_alloc_details`(`patient_id`, `ipd_id`, `ward_id`, `bed_id`, `alloc_type`, `time`, `date`, `user`) VALUES ('$patient_id','$opd_id','$b[ward_id]','$b[bed_id]','1','$time','$date','$user')");
					
					mysqli_query($link,"INSERT INTO `ipd_pat_bed_details`(`patient_id`, `ipd_id`, `ward_id`, `bed_id`, `user`, `time`, `date`) VALUES ('$patient_id','$opd_id','$b[ward_id]','$b[bed_id]','$user','$time','$date')");
					
					mysqli_query($link,"DELETE FROM `ipd_bed_details_temp` WHERE `patient_id`='$user'");
					
					//~ mysqli_query($link,"INSERT INTO `ipd_pat_info`(`patient_id`, `ipd_id`, `occupation`, `address_type`, `add_1`, `add_2`, `city`, `state`, `zip`, `country`, `phone_type`, `phone`, `email`, `insurance`) VALUES ('$patient_id','$opd_id','','','$address','','$city','$state','$pin','','','$phone','$email','0')");
					
					//~ mysqli_query($link,"INSERT INTO `ipd_pat_relation`(`patient_id`, `ipd_id`, `person_type`, `name`, `address_type`, `add_1`, `add_2`, `city`, `state`, `zip`, `country`, `phone`, `email`) VALUES ('$patient_id','$opd_id','','$gd_name','','$address','','$city','$state','$pin','','$phone','$email')");
					
					//~ mysqli_query($link,"INSERT INTO `ipd_pat_details`(`patient_id`, `ipd_id`, `user`, `time`, `date`) VALUES ('$patient_id','$opd_id','$user','$time','$date')");
					
					mysqli_query($link,"INSERT INTO `ipd_pat_doc_details`(`patient_id`, `ipd_id`, `attend_doc`, `admit_doc`, `dept_id`) VALUES ('$patient_id','$opd_id','$atdoc_id','$addoc_id','$dept_id')");
					
					mysqli_query($link,"INSERT INTO `ipd_pat_doc_transfer`(`patient_id`, `ipd_id`, `attend_doc`, `status`, `date`, `time`, `user`) VALUES ('$patient_id','$opd_id','$atdoc_id','1','$date','$time','$user')");
		...
		else
		{
			...
			if($emp_access["edit_info"]==1)
			{
				...
				mysqli_query($link, " UPDATE `uhid_and_opdid` SET `refbydoctorid`='$refbydoctorid', `center_no`='$sel_center', `hguide_id`='$hguide_id', `branch_id`='$branch_id', `urgent`='$urgent', `remarks`='$remarks', `doc_discount`='$doc_discount' WHERE `patient_id`='$patient_id' and `opd_id`='$opd_id' ");
				
				
				$pat_bed=mysqli_fetch_array(mysqli_query($link, " SELECT * FROM `ipd_pat_doc_details` WHERE `patient_id`='$patient_id' and `ipd_id`='$opd_id' "));
				...
				mysqli_query($link," UPDATE `ipd_pat_doc_details` SET `attend_doc`='$atdoc_id',`admit_doc`='$addoc_id',`dept_id`='$dept_id' WHERE `patient_id`='$patient_id' and `ipd_id`='$opd_id' ");
				
				$m_slno=mysqli_fetch_array(mysqli_query($link," SELECT MAX(`slno`) AS `max_slno` FROM `ipd_pat_doc_transfer` WHERE `patient_id`='$patient_id' AND `ipd_id`='$opd_id' "));
				
				mysqli_query($link," UPDATE `ipd_pat_doc_transfer` SET `attend_doc`='$atdoc_id', `user`='$user' WHERE `slno`='$m_slno[max_slno]' ");
			...
			{
				echo $patient_id."@".$opd_id."@".$emp_access_str."@Don't have access to update";
			}
		}
	}
}
		...
		if($now_pay>0 || $discount_amount>0 || $balance>0)
		{
			...
			else
			{
				...
				else
				{
					...
					mysqli_query($link, " INSERT INTO `payment_detail_all`(`patient_id`, `opd_id`, `transaction_no`, `bill_amount`, `already_paid`, `amount`, `discount_amount`, `discount_reason`, `refund_amount`, `refund_reason`, `tax_amount`, `tax_reason`, `balance_amount`, `balance_reason`, `payment_type`, `payment_mode`, `cheque_ref_no`, `user`, `date`, `time`) VALUES ('$patient_id','$opd_id','$bill_no','$bill_amount','$now_pay','0','0','','0','','0','','$balance','$balance_reason','Advance','$payment_mode','','$pat_reg[user]','$pat_reg[date]','$pat_reg[time]') ");
				}
			}
			echo "Updated@101";
		}
...
{
	if(mysqli_query($link, " TRUNCATE TABLE `ipd_bed_details_temp` "))
	{
		echo "Cleared";
	}
	else
