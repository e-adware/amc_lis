...
function text_query_memo_json($txt,$file)
{
	$txt_file="../../cashmemos/".$file;
	$fp = fopen($txt_file, 'w');
...
$url="http://192.168.5.1:8081/lmch-api/api/public/lis/";
//$token=$_POST['token'];
...
if($type==1)
{
	...
	preg_match_all('!\d+!', $CashMemoNo, $matches);
	$numbers_array = $matches[0];
	//print_r($numbers_array);
	$CashMemoNo=$numbers_array[0];
	//*
	...
	//echo $server_output;
	text_query_memo_json($server_output, $CashMemoNo.'.json');
	...
	$discountAmt=$resArray['discountAmt'];
	$freeType	=$resArray['freeType'];
	$AuthBy		=$resArray['freeAuthBy'];
	$PMJAY		=$resArray['PMJAY'];
	$JSSK		=$resArray['JSSK'];
	$RBSK		=$resArray['RBSK'];
	$schemeName	=$resArray['schemeName'];
	...
	$chDt		=explode(" ", $CashMemoDate);
	$cshDt		=explode("/", $chDt[0]);
	$cshDate	=date("Y-m-d", strtotime($cshDt[2]."-".$cshDt[1]."-".$cshDt[0]));
	$cshTime	=date("H:i:s", strtotime($cshDt[1]));
	...
	$freeTypeName=mysqli_fetch_array(mysqli_query($link,"SELECT `name` FROM `free_type_master` WHERE `code`='$freeType'"));
	?>
	...
	<table class="table table-condensed table-bordered table-report">
		...
		<tr>
			<th>Patient Type</th>
			<td><?php echo $patType;?></td>
			<th>Free Type</th>
			<td><?php echo $freeTypeName['name'];?></td>
			<th>Ward</th><td style="background:<?php echo $wardClass;?>;"><?php echo $ward;?></td>
		</tr>
		<tr>
			<th>AuthBy</th>
			<td><?php echo $AuthBy;?></td>
			<td colspan="2"><i class="<?php if($PMJAY){echo "icon-check";}else{ echo "icon-check-empty";}?> icon-large"></i>PMJAY &nbsp;&nbsp; <i class="<?php if($JSSK){echo "icon-check";}else{ echo "icon-check-empty";}?> icon-large"></i>JSSK &nbsp;&nbsp; <i class="<?php if($RBSK){echo "icon-check";}else{ echo "icon-check-empty";}?> icon-large"></i>RBSK</td>
			<th>Scheme Name</th>
		...
		<tr>
			<th></th><td></td>
			<th></th><td></td>
			<th>Discount</th><td><?php echo $discountAmt;?></td>
		</tr>
		<tr>
		...
		foreach($TestDetails as $ky=>$vl)
		{
			...
			$dept=mysqli_fetch_assoc(mysqli_query($link,"SELECT a.`type_id` FROM `testmaster` a, `test_department` b WHERE a.`type_id`=b.`id` AND a.`testid`='$tst[pTestId]'"));
		?>
		<tr>
			<td class="<?php echo $tdClass;?>" colspan="5"><?php echo $i.". ".$tst['testname']." (".$TestId.") ";?></td>
			<td><?php echo $Rate;?></td>
		</tr>
		<?php
	...
	<input type="hidden" id="aTestRts" value="<?php echo $aTestRts;?>" />
	<input type="hidden" id="testDepts" value="<?php echo $allDepts;?>" />
	<input type="hidden" id="patType" value="<?php echo $patType;?>" />
	<input type="hidden" id="freeType" value="<?php echo $freeType;?>" />
	<input type="hidden" id="authBy" value="<?php echo $AuthBy;?>" />
	<input type="hidden" id="schemeName" value="<?php echo $schemeName;?>" />
	<input type="hidden" id="discountAmt" value="<?php echo $discountAmt;?>" />
	<div style="display:none;">
	<input type="checkbox" id="pmjay" <?php if($PMJAY){echo "checked";}?> disabled />
	<input type="checkbox" id="jssk" <?php if($JSSK){echo "checked";}?> disabled />
	<input type="checkbox" id="rbsk" <?php if($RBSK){echo "checked";}?> disabled />
	</div>
	<?php
}
...
if($type==2)
{
	...
	$RegDt		=explode("/", $RegDate[0]);
	$regnDate	=date("Y-m-d", strtotime($RegDt[2]."-".$RegDt[1]."-".$RegDt[0]));
	$regnTime	=date("H:i:s", strtotime($RegDate[1]));
	
	$testDepts	= $_POST['testDepts'];
	$patType	= $_POST['patType'];
	$freeType	= $_POST['freeType'];
	$authBy		= $_POST['authBy'];
	...
	if($checkPat)
	{
		...
		mysqli_query($link,"UPDATE `patient_info` SET `name`='$name', `sex`='$sex', `dob`='$dob', `age`='$age', `age_type`='$ageType', `phone`='$phNo' WHERE `patient_id`='$PatientNo'");
		$txt.="\nUPDATE `patient_info` SET `name`='$name', `sex`='$sex', `dob`='$dob', `age`='$age', `age_type`='$ageType' WHERE `patient_id`='$PatientNo'";
	...
	else
	{
		...
		else
		{
			...
			mysqli_query($link,"INSERT INTO `patient_info`(`patient_id`, `uhid`, `name`, `gd_name`, `relation`, `sex`, `dob`, `age`, `age_type`, `phone`, `address`, `email`, `religion_id`, `blood_group`, `marital_status`, `occupation`, `gurdian_Occupation`, `income_id`, `education`, `gd_phone`, `pin`, `police`, `state`, `district`, `city`, `post_office`, `father_name`, `mother_name`, `file_create`, `user`, `date`, `time`) VALUES ('$patient_id','$PatientNo','$name','','','$sex','$dob','$age','$ageType','$phNo','$address','','0','','0','','','0','','','','','4','65','','','','','0','$user','$regnDate','$regnTime')");
			$txt.="\nINSERT INTO `patient_info`(`patient_id`, `uhid`, `name`, `gd_name`, `relation`, `sex`, `dob`, `age`, `age_type`, `phone`, `address`, `email`, `religion_id`, `blood_group`, `marital_status`, `occupation`, `gurdian_Occupation`, `income_id`, `education`, `gd_phone`, `pin`, `police`, `state`, `district`, `city`, `post_office`, `father_name`, `mother_name`, `file_create`, `user`, `date`, `time`) VALUES ('$patient_id','$PatientNo','$name','','','$sex','$dob','$age','$ageType','$phNo','$address','','0','','0','','','0','','','','','4','65','','','','','0','$user','$regnDate','$regnTime');";
		...
		while($disDept=mysqli_fetch_array($disDeptQry))
		{
			...
			if(mysqli_query($link,"INSERT INTO `uhid_and_opdid`(`patient_id`, `opd_id`, `hospital_no`, `refbydoctorid`, `center_no`, `ward_id`, `wardName`, `cashMemoNo`, `freeType`, `authBy`, `schemeName`, `discountAmt`, `pmjay`, `jssk`, `rbsk`, `branch_id`, `urgent`, `receipt_no`, `serial_no`, `reg_type`, `type`, `user`, `date`, `time`) VALUES ('$pid','$pid','$PatientNo','0','','0','$ward','$CashMemoNo','$freeType','$authBy','$schemeName','$discountAmt','$pmjay','$jssk','$rbsk','$branch_id','0','$patType','','$reg_type','2','$user','$rDate','$rTime')"))
			{
				$txt.="\nINSERT INTO `uhid_and_opdid`(`patient_id`, `opd_id`, `hospital_no`, `refbydoctorid`, `center_no`, `ward_id`, `wardName`, `cashMemoNo`, `freeType`, `authBy`, `schemeName`, `discountAmt`, `pmjay`, `jssk`, `rbsk`, `branch_id`, `urgent`, `receipt_no`, `serial_no`, `reg_type`, `type`, `user`, `date`, `time`) VALUES ('$pid','$pid','$PatientNo','0','','0','$ward','$CashMemoNo','$freeType','$authBy','$schemeName','$discountAmt','$pmjay','$jssk','$rbsk','$branch_id','0','$patType','','$reg_type','2','$user','$rDate','$rTime');";
				...
				mysqli_query($link,"UPDATE `uhid_and_opdid` SET `patient_id`='$patient_id', `opd_id`='$opd_id', `serial_no`='$date_serial' WHERE `patient_id`='$pid' AND `opd_id`='$pid' AND `wardName`='$ward' AND `hospital_no`='$PatientNo' AND `cashMemoNo`='$CashMemoNo' AND `date`='$rDate' AND `time`='$rTime' AND `user`='$user'");
				$txt.="\nUPDATE `uhid_and_opdid` SET `patient_id`='$patient_id', `opd_id`='$opd_id', `serial_no`='$date_serial' WHERE `patient_id`='$pid' AND `opd_id`='$pid' AND `wardName`='$ward' AND `hospital_no`='$PatientNo' AND `cashMemoNo`='$CashMemoNo' AND `date`='$rDate' AND `time`='$rTime' AND `user`='$user';";
				...
				foreach($all_dept_Tests as $test)
				{
					...
					if($test)
					{
						...
						$prefNo="";
						//-------------------------------//
						$testRate=$patTestRts[$ind];
						
						mysqli_query($link,"INSERT INTO `patient_test_details`(`patient_id`, `opd_id`, `ipd_id`, `batch_no`, `testid`, `sample_id`, `test_rate`, `test_discount`, `dept_serial`, `addon_testid`, `date`, `time`, `user`, `type`) VALUES ('$patient_id','$opd_id','','1','$test','$SampleId','$testRate','0','','0','$rDate','$rTime','$user','2')");
						$txt.="\nINSERT INTO `patient_test_details`(`patient_id`, `opd_id`, `ipd_id`, `batch_no`, `testid`, `sample_id`, `test_rate`, `test_discount`, `dept_serial`, `addon_testid`, `date`, `time`, `user`, `type`) VALUES ('$patient_id','$opd_id','','1','$test','$SampleId','$testRate','0','','0','$rDate','$rTime','$user','2');";
