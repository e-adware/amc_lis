...
<div id="content-header">
	<div class="header_div"> <span class="header"> Phelbotomy Sample Receive (OPD) </span></div>
</div>
<!--End-header-->
<div class="container-fluid">
	...
	<div style="position:fixed;right:0" id="bar_setting">
		...
		if ($chk_bar[ip_address]) {
			?><button class='btn btn-info btn-success' onclick="load_bar_setting('<?php echo $ip; ?>')"><i
					class='icon-wrench'></i></button><?php
		}
		?>
	</div>
	...
	<div id="search_det" style="display:none">
		...
		<table id="padd_tbl" class="table table-condensed">
			...
			<tr>
				<th>Sample No<br />
					<input type="text" style="width:90px;" value="OPD_BIO/" readonly>
					<input type="text" style="width:90px;" id="bill_no_upd" onkeyup="load_emp_event(event)"
				...
				<th><br />
					<input type='hidden' value="" id="patient_no_upd" onkeyup="load_emp_event(event)"
			...
			<tr>
				<th colspan="4" style="text-align:center">
					...
					<input type="button" id="search_clr" value="Clear" class="btn btn-info"
						onclick="$('#hosp_no_upd').val('');$('#bill_no_upd').val('');$('#name_upd').val('');" />
				</th>
			</tr>
			<tr>
				<td colspan="4">
	...
	<div id="reg_div" tabindex='1' onkeyup="move_focus(event)">
		...
		<table class="table table-bordered table-condensed">
			...
			<tr>
				<th>Hosp No:</th>
				<td colspan="3"><input type="text" id="hosp_no" name="c_4" placeholder="Enter Hosp. No"
						onkeyup="check_hosp_no(event)" autofocus /></td>
			...
			<tr>
				...
				<th>Phone</th>
				<th><input type="text" id="phone" name="c_7" class="no_imp"
			...
			<tr>
				<th style="width:200px">Age :<input type="text" id="age" style="width:50px" name="c_8" /> <select
						id="age_type" name="c_9" style="width:50px">
						<option value="Years">Y</option>
						<option value="Months">M</option>
						<option value="Days">D</option>
					</select></th>
				<th>Sex :<select id="sex" name="c_10" style="width:50px">
				...
				<td>
					<input type="text" style="width:90px;" value="OPD_BIO/" readonly>
					<input type="text" id="slno" name="c_11" onkeyup="chk_smp(1,event)" />
				</td>
				<th>Ward</th>
				<th>
					<select id="dept" name="c_11" style="display:none">
			...
			<tr>
				...
				<th colspan="3">
					...
					<select class="span2" id="free" name="c_14" onchange="load_auth()"
						onkeydown="select_enter(this.id,event)" autofocus>
					...
					<select class="span2" id="auth" style="display:none" onkeydown="select_enter(this.id,event)">
					...
					<select class="span1" id="auth_disc" style="display:none" onkeydown="select_enter(this.id,event)">
			...
			<tr>
				<th colspan="7">
					<div style="font-size:15px;text-align:center">Patient No: <span id="patient_no"></span></div>
				</th>
			</tr>
			<tr>
				<th colspan="7">
					<div style="border-bottom:1px solid #DDDDDD">
						<div align="center" style="border-bottom:1px solid #CCC">
							<input type="hidden" class="span5" id="srch_test" onfocus="load_tests1()"
								onblur="setTimeout(function(){$('#ref_doc').empty().hide();},600);"
								placeholder="Search Test Name" />
							<input type="text" placeholder="Search Test" onkeyup="select_test(this.value,event)"
								id="tst_search" name="c_13" onfocus="slide_test()" />
						</div>
			...
			<tr>
				<td colspan="7" style="text-align:center">
					...
					while ($smp_d = mysqli_fetch_array($sampl_det)) {
						?>
						<div style="display:inline-block;width:200px;padding:5px;background:#CCC;font-weight:bold;cursor:pointer"
							onclick="check_sample(<?php echo $smp_d[id]; ?>)"
							onmouseover="view_sample(<?php echo $smp_d[id]; ?>)">
							<!--
					<span id="span_<?php echo $smp_d[id]; ?>">
						<input type="checkbox" id="<?php echo $smp_d[id]; ?>" value="<?php echo $smp_d[id]; ?>" class="recp_smp"/> <?php echo $smp_d[name]; ?>
					</span>
-->
							<?php echo $smp_d[name]; ?>
...
<script>
	...
	function save_data(val) {
		...
		if (hosp_no == "" || name == "" || age == "" || tst_l.length == 0) {
			/*bootbox.dialog({ message: "<h5>Some Data is Missing(HNO/Name/Age/Test)</h5>"});
			setTimeout(function(){
						bootbox.hideAll();
						$("html, body").animate({ scrollTop: 30 },"slow"),1000);
	...
	function load_new() {
		document.location = "index.php?param=" + btoa(1041);
	}
