...
<div class="container-fluid">
    <table class="table table-bordered table-condensed">
        <tr>
            ...
            <td colspan="3">
                ...
                <select id="ser_samp" onchange="load_all_test()" style="float: right;">
                    <option value="0">--Select Sample--</option>
                    <?php
                    $sam_sr = mysqli_query($link, "select `testid`, `testname` from `testmaster`");
                    while ($s_s = mysqli_fetch_array($sam_sr)) {
                        echo "<option value='$s_s[testid]'>$s_s[testname]</option>";
                    }
                    ?>
                </select>
                ...
                if ($instrument_test_num > 0) {
                    ...
                    <select id="ser_equipment" onchange="load_all_test()">
                        <?php
                        $instrument_qry = mysqli_query($link, "SELECT `id`, `name` FROM `lab_instrument_master` WHERE `status`=0 ORDER  BY `id` ASC");
    ...
    <p>
        ...
        <span style="float: right; display: none;">
            <button class="btn btn-new" onClick="load_test_info(0)"><i class="icon-edit"></i> Create New
                Test</button>
        </span>
    </p>
...
<script>
    ...
    function ser_category_change() {
        $("#loader").show();
        $.post("pages/tat_update_data.php",
            {
    ...
    function test_rate_change_up(branch_id, id, e, val) {
    ...
    function load_test_info(testid) {
        ...
        $.post("pages/tat_update_details.php",
            {
                testid: testid
            },
            function (data, status) {
        ...
        $.post("pages/tat_update_data.php",
            {
                ...
                type_id: $("#type_id").val(),
                min_time: $("#minimum_time").val(),
                max_time: $("#maximum_time").val(),
                branch_id: $("#branch_id").val(),
                user: $("#user").text().trim(),
