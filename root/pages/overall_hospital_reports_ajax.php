...
if ($_POST["type"] == 3) {
    ...
    <div style="text-align:right;">
        <button type="button" class="btn btn-primary" id="act_btn"
            onclick="print_report('<?php echo $fdate; ?>','<?php echo $tdate; ?>','<?php echo $type; ?>')">Print</button>
    </div>
    <div>
    ...
    <table class="table table-condensed table-report">
        ...
        $pat_reg_qry = mysqli_query($link, "SELECT DATEDIFF(b.date,STR_TO_DATE(a.dob, '%d-%m-%Y')) AS `age_in_days`, STR_TO_DATE(a.dob, '%d-%m-%Y') AS `dob`,b.date AS `reg_date`,a.sex FROM `patient_info` a, `uhid_and_opdid` b WHERE a.patient_id=b.patient_id AND b.`type`= 3 AND b.`date` BETWEEN '$fdate' AND '$tdate'");

        while ($pat_reg = mysqli_fetch_array($pat_reg_qry)) {
            if ($pat_reg["age_in_days"] < 31) {
                if ($pat_reg["sex"] == "M") {
                    $new_born_male++;
                } else if ($pat_reg["sex"] == "F") {
                    $new_born_female++;
                } else if ($pat_reg["sex"] == "O") {
                    $new_born_other++;
                }
                $new_born++;
