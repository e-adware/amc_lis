...
if($type==1)
{
	...
	if($val!='')
	{
		$dis_lst=mysqli_query($link,"select * from disease_master where name like '%$val%' order by name");
	}
	else
	{
		$dis_lst=mysqli_query($link,"select * from disease_master order by name");
	}
	?>
...
else if($type==2)
{
	...
	if($id=='')
	{
		mysqli_query($link,"insert into disease_master(name) value('$name')");	
	}
	else
	{
		mysqli_query($link,"update disease_master set name='$name' where id='$id'");
	}
}
...
{
	$val=$_POST['val'];
	mysqli_query($link,"delete from disease_master where id='$val'");
}
?>
