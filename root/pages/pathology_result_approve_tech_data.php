...
if ($type == "load_pat_list") {
	...
	if ($flaggedPatient) {
		...
		if ($uhid != "") {
			$test_str = "SELECT a.`patient_id`,a.`opd_id`,a.`ipd_id`,a.`batch_no` FROM `phlebo_sample` a , `uhid_and_opdid` b, `testmaster` c, `patient_info` d WHERE a.`patient_id`=b.`patient_id` AND a.`patient_id`=d.`patient_id` AND (a.`opd_id`=b.`opd_id` OR a.`ipd_id`=b.`opd_id`) AND a.`testid`=c.`testid`";
			$test_str .= " AND d.`hosp_no` LIKE '$uhid%'";
...
if ($type == "load_pat_list_multi_print") {
	$pat_type = $_POST['pat_type'];
	$fdate = $_POST['fdate'];
	$tdate = $_POST['tdate'];
	$dept_id = $_POST['dept_id'];
	...
	//$test_str="SELECT a.* FROM `testresults` a, `uhid_and_opdid` b WHERE a.`patient_id`=b.`patient_id` AND a.`opd_id`=b.`opd_id` AND a.`date` BETWEEN '$fdate' AND '$tdate'";
	$test_str = "SELECT a.*,c.name FROM `testresults` a, `uhid_and_opdid` b,patient_info c WHERE a.`patient_id`=b.`patient_id` AND a.`opd_id`=b.`opd_id` and a.`patient_id`=c.`patient_id` AND a.`date` BETWEEN '$fdate' AND '$tdate'";
	...
	if ($barcode_id != "") {
		$test_str .= " AND b.`cashMemoNo`='$barcode_id'";

		$list_start = 50;
	}
	if ($name != "") {

		$test_str .= " AND c.`name`like '%$name%'";
	...
	<table class="table table-bordered table-condensed" style="background-color:white;">
		...
		while ($test_det = mysqli_fetch_array($test_qry)) {
			...
			<tr class="<?php echo $flag_class; ?> <?php echo $printClass; ?>" id="<?php echo $tr_id; ?>">
				...
				if ($barcode_num > 0) {
					//echo "<td>";
					//echo $test_det['opd_id'];
					//echo "</td>";
				}
				$tstNames = "";
				$tstQry = mysqli_query($link, "SELECT DISTINCT `testid` FROM `testresults` WHERE `patient_id`='$patient_id' AND `opd_id`='$opd_id' AND `ipd_id`='$ipd_id' AND `batch_no`='$batch_no' AND `date` BETWEEN '$fdate' AND '$tdate'");
				while ($tt = mysqli_fetch_array($tstQry)) {
					$tst = mysqli_fetch_array(mysqli_query($link, "SELECT `testname` FROM `testmaster` WHERE `testid`='$tt[testid]'"));
					if ($tstNames) {
						$tstNames .= ", " . $tst['testname'];
					} else {
						$tstNames = $tst['testname'];
					}
				}
...
if ($type == "load_pat_dept_tests") {
	...
	<div id="pat_test_data">
		<div class="pat_info">
			<table class="table table-condensed table-bordered" style="background-color: white;">
				...
				<tr>
					...
					<td><?php echo $age . "-" . $pat_info["sex"]; ?></td>
					<td><?php echo date("d-m-Y", strtotime($test_date)) . "/" . date("h:i:s A", strtotime($test_time)); ?>
					</td>
		...
		<div>
			<b>Hot Key :</b>
			<span
				style="background-color: white;font-weight: bold;color: red;border: 1px solid white;border-radius: 10px;padding: 3px;">CTRL+ENTER</span>
			To <b>Validate</b>
			<span
				style="background-color: white;font-weight: bold;color: red;border: 1px solid white;border-radius: 10px;padding: 3px;">ESC</span>
			To <b>Back</b>
		</div>
			...
			<div class="widget-title">
				<ul class="nav nav-tabs">
					...
					while ($dept_info = mysqli_fetch_array($dept_qry)) {
						...
						?>
						<li class="li_cls <?php echo $active_cls; ?>" id="li_dept_<?php echo $dept_id; ?>"
							onclick="load_pat_dept_tests_refresh('<?php echo $dept_id; ?>')" title="Click to Re-Fresh"><a
								data-toggle="tab" href="#tab<?php echo $tab; ?>"><?php echo $dept_info["name"]; ?></a></li>
						<?php
						$tab++;
					}
					?>
			...
			<div class="widget-content tab-content" style="background-color: white;">
				...
				while ($dept_info = mysqli_fetch_array($dept_qry)) {
					...
					<div id="tab<?php echo $tab; ?>" class="tab-pane <?php echo $active_cls; ?>">
						<?php
						if ($sel_dept_id == $dept_id) {
							include("pathology_result_approve_tech_load_param.php");
						}
						?>
					</div>
					<?php
		...
		<script>
			...
			// Save Param
			function each_param_save(testid, paramid, iso_no, param_name, dept_id) {
				//~ if($("#"+testid+"PRM"+paramid).val().trim()=="")
				//~ {
				//~ alertmsg("Nothing to save", 0);
				...
				$.post("pages/pathology_result_approve_tech_ajax.php",
					{
						...
						paramid: paramid,
						iso_no: iso_no,
						instrument_id: $("#" + testid + "PRM" + paramid).attr("instrument_id"),
						result: $("#" + testid + "PRM" + paramid).val().trim(),
						dontPrint: $("#dontPrint_param" + testid + "tst" + paramid + ":checked").length,
					},
					function (data, status) {
						//alert(data);
			...
			function each_param_sample_result_delete(testid, paramid, iso_no) {
				//~ if($("#"+testid+"PRM"+paramid).val().trim()=="")
				//~ {
				//~ alertmsg("Nothing to save", 0);
				//~ $("#"+testid+"PRM"+paramid).focus();
				...
				$.post("pages/pathology_result_approve_tech_ajax.php",
					{
						type: "each_param_sample_result_delete",
						patient_id: $("#patient_id").val(),
						opd_id: $("#opd_id").val(),
						ipd_id: $("#ipd_id").val(),
						batch_no: $("#batch_no").val(),
			...
			function each_test_param_result_save(testid) {
				var param = $('input[param_testid="' + testid + '"]');

				var dataArray = [];
				var paramid_1_non_result = "";

				for (var i = 0; i < param.length; i++) {
					var testparam_ids = param[i].id;
					var testparam = testparam_ids.split("PRM");
					...
					var mandatory = $("#" + testid + "PRM" + paramid).attr("mandatory");
					...
					if ($("#approve_val").val() == 1) // Save data sent to server
					{
						load_pat_dept_tests_refresh($("#sel_dept_id").val());
						return false;
					}
					bootbox.hideAll();
					bootbox.dialog({
						message: "<h5>Are you sure want to save results ?</h5>",
						buttons: {
							cancel: {
							...
							confirm: {
								...
								callback: function () {
									...
									$.post("pages/pathology_result_approve_tech_ajax.php",
										{
											type: "each_test_param_result_save",
											patient_id: $("#patient_id").val(),
											opd_id: $("#opd_id").val(),
											ipd_id: $("#ipd_id").val(),
											batch_no: $("#batch_no").val(),
			...
			function approve_data() {
				...
				for (var i = 0; i < param.length; i++) {
					...
					if (mandatory == 1 && (result == "" || !result)) {
						alertmsg("Enter result of Mandatory parameter", 0);
						setTimeout(function () {
							$("#" + testid + "PRM" + paramid).focus();
						}, 100);
						return false;
				...
				bootbox.hideAll();
				bootbox.dialog({
					message: "<h5>Are you sure want to validate all result(s) ?</h5>",
					buttons: {
						cancel: {
							label: '<i class="icon-remove"></i> No (ESC)',
							className: "btn btn-inverse",
			...
			function approve_param_each(testid, paramid, iso_no, param_name, dept_id) {
				...
				if ($("#" + testid + "T" + paramid + ":checked").length == 1) {
					...
					var msg = "Are you sure want to validate " + param_name + " ?";

					val = 1;
				} else {
					var msg = "Are you sure want to in-validate " + param_name + " ?";
			...
			function approve_test_summ_each(testid, iso_no, testname, dept_id) {
				...
				if ($("#approve_test_summary" + testid + ":checked").length == 1) {
					var msg = "Are you sure want to validate summary of " + testname + " ?";

					val = 1;
				} else {
					var msg = "Are you sure want to in-validate summary of " + testname + " ?";
			...
			function approve_widal_click(testid, dept_id) {
				...
				if ($("#widal_chk:checked").length == 1) {
					var msg = "Are you sure want to validate ?";

					val = 1;
				} else {
					var msg = "Are you sure want to in-validate ?";
			...
			function load_culture_data(testid) {
				if ($("#growth_val" + testid).val() == 2) // Growth
				{
					...
					$("#test_summary_label" + testid).hide();
					$("#summary_display_sub_div" + testid).hide();
				}
				else {
					...
					$("#test_summary_label" + testid).show();
					$("#summary_display_sub_div" + testid).show();
				}

				$.post("pages/pathology_result_approve_tech_culture_data.php", {
...
if ($type == "refresh_pat_dept_color") {
	...
	while ($dept = mysqli_fetch_array($dept_qry)) {
		...
		// No Main Tech Approve
		$non_approve_testresults = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(a.slno) AS `tot` FROM `testresults` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`=0"));

		$non_approve_summary = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(a.slno) AS `tot` FROM `patient_test_summary` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`=0"));

		$non_approve_widal["tot"] = 0;
		if ($type_id == 32) // Serology
		{
			$non_approve_widal = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(DISTINCT a.`testid`) AS `tot` FROM `widalresult` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`=0 LIMIT 1"));
		}
		...
		// Main Tech Approve
		$approved_testresults = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(a.slno) AS `tot` FROM `testresults` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`>0"));

		$approved_summary = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(a.slno) AS `tot` FROM `patient_test_summary` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`>0"));

		$approved_widal["tot"] = 0;
		if ($type_id == 32) // Serology
		{
			$approved_widal = mysqli_fetch_array(mysqli_query($link, "SELECT COUNT(DISTINCT a.`testid`) AS `tot` FROM `widalresult` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$patient_id' AND a.`opd_id`='$opd_id' AND a.`ipd_id`='$ipd_id' AND a.`batch_no`='$batch_no' AND b.`type_id`='$type_id' AND a.`main_tech`>0 LIMIT 1"));
		}
		...
		$dept_param_str = "SELECT COUNT(DISTINCT a.`ParamaterId`) AS `tot_param` FROM `Testparameter` a, `testmaster` b , `phlebo_sample` c , `Parameter_old` d WHERE a.`TestId`=b.`testid` AND c.`TestId`=b.`testid` AND  a.`status`=0 AND a.`ParamaterId`=d.`ID` AND d.`ResultType` NOT IN(0,5) AND c.patient_id = '$patient_id' AND c.opd_id = '$opd_id' AND c.ipd_id = '$ipd_id' AND c.batch_no = '$batch_no' AND b.type_id = '$type_id'";
