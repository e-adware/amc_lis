		...
		<table class="table table-condensed">
			<tr>
				<td colspan="5">
					<center>
						...
						<input class="form-control datepicker span2" type="text" name="fdate" id="fdate"
							value="<?php echo date("Y-m-d"); ?>" style="margin-left: 47px;">
						<span class="side_name">To</span>
						<input class="form-control datepicker span2" type="text" name="tdate" id="tdate"
							value="<?php echo date("Y-m-d"); ?>" style="margin-left: 26px;">

						<select id="pat_type" name="" style="display:none;" class="span2">
						...
						<span id="dep_tst_lst">
							<select id="search_testid" name="search_testid" class="span2" style="display:none;">
			...
			<tr>
				<td style="text-align:center;font-weight:bold">
					...
					Sample Type<br>
					<select id="type_prefix" class="span2" onchange="load_pat_ser()">
					...
					<input onkeyup="load_pat_ser()" type="text" id="sample_serial" class="span1" />
				</td>
				<td style="text-align:center;font-weight:bold">
					<!--Cash Memo No.--> <br /> <input type="text" id="bill_no" style="display:none;"
						onkeyup="load_pat_event(event)" />
					<select class="span2" id="patType" onchange="load_pat_ser(0)" style="display:none;">
				...
				<td style="text-align:center;font-weight:bold">
					<br>
					<select id="dept_serial" class="span1" style="display:none;" onchange="load_pat_ser(1)">
						<option value="O">O</option>
						<option value="I">I</option>
						<option value="E">E</option>
					</select>
					<input type="text" id="dept_serial_no" class="span1" style="display:none;"
						onkeyup="dept_serial_no_up(event)">
				</td>
				<td style="text-align:center;font-weight:bold">
					Name <br />
...
<button id="btn_modal_note" type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal"
	style="display:none;">Open Modal Note</button>
...
<script>
	...
	function unflag_pat(uhid, opd, ipd, batch, dept) {
		//~ bootbox.dialog({
		//~ message: "<h5>Un-Flag patient ?</h5>",
		//~ closeButton: false,
		//~ buttons: {
		//~ cancel: {
		...
		bootbox.prompt({
			...
			callback: function (result) {
				if (result === null) {
					...
					$(".textAlertMsg").remove();
					// Show error message and refocus
					$(".bootbox-input-text")
						.focus()
						.closest(".bootbox-body")
						.append('<div class="text-danger textAlertMsg">Please enter a value.</div>');
					return false;
				} else {
					unflag_confirm(uhid, opd, ipd, batch, dept, result);
				}
			}
		}).on("shown.bs.modal", function () {
			// Target the input after modal is fully rendered
			$(this).find(".bootbox-input")
				.addClass("span4")
				.attr("placeholder", "Remarks");
		});
	}
	function unflag_confirm(uhid, opd, ipd, batch, dept, result) {
		$("#loader").show();
		$.post("pages/flagged_pat_list_ajax.php",
			{
				uhid: uhid,
