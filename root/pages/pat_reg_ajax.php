...
if($type==1)
{
	...
	{
		?>
			<div class="tst_span <?php echo strtolower($tst[testname]);?> <?php echo $tst[type_id];?>" id="div_<?php echo $i;?>" onclick="select_check(<?php echo $i;?>)" > <input type="checkbox" value="<?php echo $tst[testid];?>" id="check_<?php echo $i;?>" class="tst_check"/> <label><span></span></label> <?php echo $tst[testname];?> </div>
		<?php
		$i++;				
	}
	?> </div> <?php			
...
else if($type==2)
{
	...
	else if($val=="Update")
	{
		...
		mysqli_query($link,"update uhid_and_opdid set type='$pat_type', hosp_no='$hosp_no',pat_type='$free',ward='$ward',dept='$dept' where patient_id='$det[patient_id]' and opd_id='$opd_id'");		
...
else if($type==11)
{
	...
	?>
	<div class="btn-group">
		<button class="btn btn-success" <?php echo $plus;?> onclick="bar_change('<?php echo $ip;?>',1)" id="bar_plus"><i class="icon-plus"></i></button>
		<input type="text" value="<?php echo $val[fo_val];?>" id="bar_value" onkeyup="change_bar(event,'<?php echo $ip;?>')" class="span1" style="margin-top:10px"/>
		<button class="btn btn-success" <?php echo $minus;?>  onclick="bar_change('<?php echo $ip;?>',2)"  id="bar_plus"><i class="icon-minus"></i></button>
