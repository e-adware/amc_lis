<style>
	...
	#testDetailsTable th:hover {
		background-color: #f5f5f5;
	}

	#testDetailsTable th.sorted-asc::after {
		content: "↑";
		position: absolute;
		right: 8px;
		font-size: 12px;
	...
	#testDetailsTable th.sorted-desc::after {
		content: "↓";
		position: absolute;
		right: 8px;
		font-size: 12px;
			...
			<td>
				<center>
					...
					<input class="form-control datepicker span2" type="text" name="to" id="to"
						value="<?php echo date("Y-m-d"); ?>" readonly>
					<select class="span3" id="testid" onchange="view_all('head_wise_test_detail')">
						<option value="">Select Test</option>
						<?php
						$q = mysqli_query($link, "SELECT `testid`,`testname` FROM `testmaster` ORDER BY `testname`");
					...
					<button class="btn btn-success" onClick="view_all('head_wise_test_detail')"><i
							class="icon-search"></i> Test wise Details</button>
				</center>
			</td>
		</tr>
	</table>
...
<script>
	$(document).ready(function () {
		...
		view_all('head_wise_test_detail');
	...
	function print_page(val, date1, date2, head_id, encounter, branch_id, testid) {
		...
		url = "pages/head_wise_detail_pat_print.php?date1=" + date1 + "&date2=" + date2 + "&head_id=" + head_id + "&encounter=" + encounter + "&branch_id=" + branch_id + "&val=" + val + "&testid=" + testid;
	...
	function sortTable(columnIndex, tableId, isNumeric = false) {
		...
		rows.sort((a, b) => {
			...
			if (isNumeric) {
				...
				// For Slno. column, compare as numbers
				if (columnIndex === 0) {
