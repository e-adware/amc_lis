...
$list_start=$_POST['list_start'];
	...
	$str="select a.`opd_id` from patient_test_details a,testmaster b where a.testid=b.testid and b.category_id='1' and a.date between '$from' and '$to'";


if($pat_name!='')
{
	$str="select a.`opd_id` from patient_test_details a,testmaster b,patient_info c where a.patient_id=c.patient_id and a.testid=b.testid and b.category_id='1' and c.name like '$pat_name%'";
	$list_start=50;
}
if($patientNo!='')
{
	$str="select a.`opd_id` from patient_test_details a,testmaster b where a.patient_id='$patientNo' and a.testid=b.testid and b.category_id='1' and a.date between '$from' and '$to'";
	$list_start=50;
}
if($barcode!='')
{
	$str="select a.`opd_id` from uhid_and_opdid a,patient_test_details b where a.cashMemoNo='$barcode' and a.opd_id=b.opd_id ";
	//$str="select a.`opd_id` from patient_test_details a,testmaster b where a.opd_id='$barcode' and a.testid=b.testid and b.category_id='1'";
	$list_start=50;
}

$str.=" GROUP BY a.opd_id ORDER BY a.`slno` DESC";
if($list_start>0)
{
	$str.=" limit ".$list_start;
}
//echo $str;
...
<?php
	...
	while($valu=mysqli_fetch_array($qryy))
	{
		...
		if($display=="Yes")
		{
			...
			$batch_qry=mysqli_query($link, " SELECT a.`batch_no` FROM `patient_test_details` a, `testmaster` b WHERE a.`testid`=b.`testid` AND a.`patient_id`='$pat_info[patient_id]' and a.`opd_id`='$valu[opd_id]' and a.`ipd_id`='$valu[ipd_id]' group by a.`batch_no` order by `slno` DESC ");
			...
			while($batch_val=mysqli_fetch_array($batch_qry))
			{
				...
				if($phlebo_num==0)
				{
					...
					$checkRej=mysqli_fetch_array(mysqli_query($link,"SELECT COUNT(`slno`) AS `counts` FROM `phlebo_sample_status` WHERE `patient_id`='$pat_info[patient_id]' and `opd_id`='$valu[opd_id]' and `ipd_id`='$valu[ipd_id]' and `batch_no`='$batch_val[batch_no]'"));
					if($checkRej['counts']>0)
					{
						$rejCount=$checkRej['counts'];
					}
				...
				$sl=mysqli_fetch_array(mysqli_query($link,"SELECT `dept_serial` FROM `patient_test_details` WHERE `patient_id`='$pat_info[patient_id]' AND `opd_id`='$valu[opd_id]' AND `ipd_id`='$valu[ipd_id]' AND `batch_no`='$batch_val[batch_no]' AND `dept_serial`!='' LIMIT 0,1"));
				if($b_num==1)
				{
		?>
...
-->
				<td class="serial_num" rowspan="<?php echo $batch_num; ?>"><?php echo $sl['dept_serial']; ?></td>
				<td rowspan="<?php echo $batch_num; ?>"><?php echo $pat_info["name"]; ?><span style="float:right;"><?php echo $img;?></span></td>
				<td rowspan="<?php echo $batch_num; ?>"><?php echo $age."/".$pat_info["sex"]; ?></td>
				<td><span class="btn_round" style="<?php echo $style_span; ?>"><?php echo $batch_test_time["batch_no"]; ?></span><span><?php if($rejCount>0){ echo "<img src='../images/vial.png' style='width:18px;float:right;' alt='' />";}?></span></td>
