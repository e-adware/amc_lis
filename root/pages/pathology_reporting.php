...
<div class="container-fluid">
	<table class="table table-bordered table-condensed">
		<tr>
			<td colspan="2">
				...
				<button class="btn btn-search" onClick="load_pat_details()" style="margin-bottom: 10px;"><i
						class="icon-search"></i> View</button>
			</td>
		</tr>
		<tr>
			<td colspan="2">
				<span class="side_name">Enter</span>
				<select id="search_type" class="span2" style="margin-left: 50px;">
					<option value="bill_no">Bill No.</option>
					<option value="uhid">UHID No.</option>
					<option value="name">NAME</option>
				</select>
				<input type="text" id="search_val" onkeyup="load_pat_info(this.value,event)" autofocus>
		...
		<tr>
			<td colspan="2">
				...
				<div id="pathology_reporting_test_list" style="display:none">
	...
	<button type="button" class="btn btn-info btn-lg" id="modal_btn_repeat" data-toggle="modal"
		data-target="#myModal_repeat" style="display:none;">Repeat Param</button>
...
<script>
	...
	function instrument_change() {
		var uhid = $("#pat_uhid").val().trim();
		var opd_id = $("#pat_opd_id").val().trim();
		var ipd_id = $("#pat_ipd_id").val().trim();
		var batch_no = $("#pat_batch_no").val().trim();
		
		//alert(uhid+" = "+opd_id+" = "+ipd_id+" = "+batch_no+" OK");
		
		load_test_detail(uhid, opd_id, ipd_id, batch_no);
	...
	function load_pat_info(val, e) {
		...
		if (unicode == 13) {
			...
			load_test_detail(pinfo[1], pinfo[2], pinfo[3], pinfo[4]);
		} else if (unicode == 40) {
			var chk = sel_pser + 1;
			...
			if (val.length > 1) {
				$.post("pages/pathology_reporting_pat_list.php", {
					search_val: val,
	...
	function load_pat_details() {
		$.post("pages/pathology_reporting_pat_list.php", {
			...
			function (data, status) {
				...
				$("#pathology_reporting_test_list").hide();
				var sel_pser = 1;
	...
	var s_tst = 1;
	var s_tst_div = 0;
	function load_test_detail(uhid, opd_id, ipd_id, batch_no) {
		//alert(uhid+" = "+opd_id+" = "+ipd_id+" = "+batch_no);
		$.post("pages/pathology_reporting_test_list.php", {
			uhid: uhid,
			opd_id: opd_id,
			ipd_id: ipd_id,
			batch_no: batch_no,
			...
			function (data, status) {
				$("#pathology_reporting_test_list").html(data);
				$("#patient_info").slideUp(500, function () {
					$("#pathology_reporting_test_list").fadeIn(500, function () {
	...
	function path_select_test(val, e) {
		...
		if (unicode == 13) {
			...
			$.post("pages/pathology_reporting_test_list.php", {
				uhid: $("#pat_uhid").val().trim(),
				opd_id: $("#pat_opd_id").val().trim(),
				ipd_id: $("#pat_ipd_id").val().trim(),
				batch_no: $("#pat_batch_no").val().trim(),
				val: val
			},
				function (data, status) {
					$("#pathology_reporting_test_list").html(data);
	...
	function load_test_param(tinfo, testid) {
		...
		$.post("pages/pathology_reporting_load_param.php", {
			uhid: $("#pat_uhid").val().trim(),
			opd_id: $("#pat_opd_id").val().trim(),
			ipd_id: $("#pat_ipd_id").val().trim(),
			batch_no: $("#pat_batch_no").val().trim(),
	...
	function save_test_param(testid, validate) {
		...
		for (var i = 0; i < par.length; i++) {
			if (par[i].id != "Save") {
				...
				if (par[i].getAttribute("padd") == 1) {
					if ($("#padd" + par[i].id + " #rad_res").contents().find('body').html()) {
						//alert($("#padd"+par[i].id+" #rad_res").contents().find('body').html());

						all = all + $("#padd" + par[i].id + " #rad_res").contents().find('body').html() + "(*TPR*)" + par[i].id + "##TPR##";
					}
				}
				else {
					if (par[i].value) {
						all = all + par[i].value + "(*TPR*)" + par[i].id + "##TPR##";
					}
				}
			}
		}
		...
		if (chk == 0) {
			$.post("pages/pathology_reporting_save.php", {
				type: "save_test_param_result",
				uhid: $("#pat_uhid").val().trim(),
				opd_id: $("#pat_opd_id").val().trim(),
				ipd_id: $("#pat_ipd_id").val().trim(),
				batch_no: $("#pat_batch_no").val().trim(),
				...
				function (data, status) {
					...
					//load_test_detail($("#pat_uhid").val().trim(), $("#pat_opd_id").val().trim(),$("#pat_ipd_id").val().trim(), $("#pat_batch_no").val().trim());
					// alert("Saved");
					if (validate == "1") {
				...
				if (par[i].value) {
					...
					if ($("#" + pid + "_mic").length > 0) {
						if ($("#" + pid + "_mic").val().trim() != '0') {
							all = all + par[i].value.toUpperCase() + " (" + $("#" + pid + "_mic").val() + ")(*TPR*)" + par[i].id + "##TPR##";
						} else {
							all = all + par[i].value + "(*TPR*)" + par[i].id + "##TPR##";
						}
					} else {
	...
	function main_save_widal(ov, hv, ahv, bhv, testid, validate) {
		$.post("pages/pathology_reporting_save.php", {
			...
			function (data, status) {
				var pin = $("#pat_opd_id").val().trim();
				if ($("#pat_ipd_id").val().trim() != '') {
					pin = $("#pat_ipd_id").val().trim();
				}
				load_test_detail($("#pat_uhid").val().trim(), $("#pat_opd_id").val().trim(), $("#pat_ipd_id").val().trim(), $("#pat_batch_no").val().trim());
				// alert("Saved");
				if (validate == 0) {
	...
	function group_print() {
		$("#g_print").hide();
		$.post("pages/pathology_reporting_group_print_test_list.php", {
			uhid: $("#pat_uhid").val().trim(),
			opd_id: $("#pat_opd_id").val().trim(),
			ipd_id: $("#pat_ipd_id").val().trim(),
			batch_no: $("#pat_batch_no").val().trim(),
	...
	function add_image(testid) {
		$.post("pages/pathology_reporting_add_image.php", {
			uhid: $("#pat_uhid").val().trim(),
			opd_id: $("#pat_opd_id").val().trim(),
			ipd_id: $("#pat_ipd_id").val().trim(),
			batch_no: $("#pat_batch_no").val().trim(),
			...
			function (data, status) {
				...
				var uploader = new qq.FineUploader({
					...
					batch_no: $("#pat_batch_no").val().trim(),
					request: {
						endpoint: 'pages/pathology_reporting_upload_image.php',
						params: {
							uhid: $("#pat_uhid").val().trim(),
							opd_id: $("#pat_opd_id").val().trim(),
							ipd_id: $("#pat_ipd_id").val().trim(),
	...
	function add_summary(tid) {
		$.post("pages/pathology_reporting_add_summary.php", {
			uhid: $("#pat_uhid").val().trim(),
			opd_id: $("#pat_opd_id").val().trim(),
			ipd_id: $("#pat_ipd_id").val().trim(),
			batch_no: $("#pat_batch_no").val().trim(),
	...
	function add_pad_param(slno, paramid) {
	...
	function save_summary(testid, validate) {
		...
		$.post("pages/pathology_reporting_save.php", {
			...
			doc: $("#doc_list").val(),
			instrument_id: $("#instrument_id").val(),
			summary: $("#rad_res").contents().find('body').html()
		},
			function (data, status) {
				//$("#summary_div").hide();
				//alert(data);
				...
				// $("#btn_" + tid).click();
				//load_test_detail($("#pat_uhid").val().trim(), pin, $("#pat_batch_no").val().trim());
				//alert("Saved");
				if (validate == "1") {
					alertmsg("Saved & Validated", 1);
				}
	...
	function test_summary_back(testid) {
		//load_test_param1($("#ival").val(), testid);
		setTimeout(function () {
			$("#btn_" + testid).click();
		}, 500);
		setTimeout(function () {
	...
	function cult_tab_click(iso_no) {
		$.post("pages/pathology_reporting_culture_data.php", {
				...
				main: {
					...
					callback: function () {
						if ($("#repeat_reason").val() != "") {
							$.post("pages/pathology_reporting_repeat_param.php",
								{
