...
<head>
	<title>
		<?php echo $page_name . " | " . $company["name"]; ?>
	</title>
	<meta charset="UTF-8" />
	...
	<script>
		...
		function hide_menu() {
			...
			$("#sidebar").animate({ width: "70px" }, "200", function () { $("#sidebar").css({ 'overflow': 'hidden' }); });
	...
	<style>
		...
		#user-nav .navbar-inverse .nav>li>a:hover,
		#user-nav .navbar-inverse .nav>li>a:focus {
			color: white;
		}
...
<body onkeyup="hid_div(event)" onload="checkCookie()">
	...
	$today = date("Y-m-d");
	$last_day = $our_client["c_date"];
	$diff = date("Y", strtotime($today)) - date("Y", strtotime($last_day));
	?>
	...
	<div class="navbar navbar-fixed-top" style="z-index: 10;">
		<div class="navbar-inner">
			<div class="container">
				<div
					style="font-size:18px; top: 10px; position: relative;color:#eeeeee;font-weight:bold;text-align: center;">
					<?php echo $our_client['name']; ?>
				</div>
				<div id="user-nav" class="navbar navbar-inverse" style="display:none;left:auto;">
					<ul class="nav" style="width: auto; margin: 0px;">
						<li class="dropdown" id="alr_dropdown">
							<a title="" href="#" data-toggle="dropdown" data-target="#alr_dropdown"
							...
							<ul class="dropdown-menu">
								<li onclick="notify_alert()" id="li_exp" style="display:none;"><a href="#"><i
											class="icon-warning-sign"></i> Expiry Notification <span
											class="badge badge-important" id="not_exp"></span></a></li>
								<li onclick="notify_low_alr()" id="li_stk" style="display:none;"><a href="#"><i
											class="icon-exclamation-sign"></i> Low Stock <span
											class="badge badge-important" id="not_stk"></span></a></li>
							</ul>
						</li>
					</ul>
				</div>
				<div id="search" class="dropdown" style="line-height: 12px;">
					<b class="dropbtn"><span class="text" style="font-size:14px;">Welcome <?php echo $p_info["name"]; ?>
						</span><span id="user" style="display:none;">
							<?php echo $_SESSION["emp_id"]; ?>
						</span>
					</b>
					<div class="dropdown-content">
						<a href="#" onCLick="pass_change_popup()"><i class="icon-check"></i> Change Password</a>
						<a href="../logout.php?Piuoi87yL8jhjUyl=<?php echo $emp_id; ?>"><i class="icon-key"></i> Log
							Out</a>
					</div>
				</div>
					...
					<div class="modal-body">
						<table class="table table-no-top-border">
							...
							<tr>
								...
								<td>
									<input type="password" name="confrm_pass" id="confrm_pass"
										onKeyUp="confrm_pass(this.value,event)"><br>
							...
							<tr>
								...
								<td>
									<input type="password" name="old_pass" id="old_pass"
										onKeyUp="old_pass(this.value,event)"><br>
							...
							<tr>
								<td colspan="2" style="text-align: center;">
									<button class="btn btn-info" id="chng_pass"
										onClick="change_password()">Change</button>
									<button class="btn btn-danger" data-dismiss="modal" class="close"
	...
	?>
	<div id="sidebar" onmousemove="show_menu_scroll()" onclick="show_menu()" onmouseleave="hide_menu()"
		style="position: fixed; top: 0%; overflow: hidden; width: 45px; z-index: 99998; height: 100%; background-color: #1e242b">
		<a href="./" class="visible-phone"><i class="icon icon-home"></i> Dashboard</a>
		<ul class="sidemenu">
			<li class="licls <?php echo $active_class; ?>">
				<a href="./" data-toggle="tooltip" data-placement="right" title="Dashboard">
					<?php echo "<div class='shortName'>D</div>"; ?> <i class="icon icon-home"></i>
					<span>Dashboard</span>
				</a>
			</li>
			<?php
			...
			while ($menu_header = mysqli_fetch_array($menu_header_qry)) {
				if ($main_header["header"] == $menu_header["id"]) {
					$active_class = "active";
				} else {
					$active_class = "";
				}
				...
				?>
				<li class="licls submenu <?php echo $active_class; ?>"><a href="#" data-toggle="tooltip"
						data-placement="right" title="<?php echo $menu_header["name"]; ?>">
						<div class="menu-item-wrapper">
							<div class="shortName"><?php echo $acronym; ?></div>
							<div class="menu-text">
								<i class="icon icon-arrow-right icon-mini"></i>
								<span><?php echo $menu_header["name"]; ?></span>
							</div>
						</div>
						...
						while ($menu_sub_header = mysqli_fetch_array($menu_sub_header_qry)) {
							...
							if ($display == "Yes") {
								...
								if ($m_num > 0) {
									...
									?>
									<li class="<?php echo $menu_active; ?>"><a href="?param=<?php echo $enc_para; ?>">
											<?php echo $menu_sub_header["par_name"]; ?>
										</a></li>
									<?php
								}
							}
	...
	<!--main-container-part-->
	<div id="content" style="width:97%;margin-left:66px;margin-right:0; margin-top: 80px;" onmouseenter="hide_menu()">
		<?php if ($our_client["n_date"] != "0000-00-00") {
			include("pages/payment_received_ncheck.php");
		} ?>