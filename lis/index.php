...
function text_query_error($dir,$txt,$file)
{
	$txt_file="../".$dir."/".$file;
	$fp = fopen($txt_file, 'w');
	if(file_exists($txt_file))
...
curl_close($ch);
//*/
//$server_output = file_get_contents('../barcodes/20250514105300.json');
//$server_output;
...
foreach($BarcodeDetails as $key=>$val)
{
	...
	foreach($TestDetails as $ky=>$vl)
	{
		...
		$testCheck=mysqli_fetch_array(mysqli_query($link,"SELECT `slno` FROM `patient_test_details` WHERE `patient_id`='$pid' AND `opd_id`='$barcode' AND `testid`='$TestId'"));
		if(!$testCheck && $TestId>0)
		{
			...
			mysqli_query($link,"INSERT INTO `patient_test_details`(`patient_id`, `opd_id`, `ipd_id`, `batch_no`, `testid`, `sample_id`, `test_rate`, `test_discount`, `dept_serial`, `addon_testid`, `date`, `time`, `user`, `type`) VALUES ('$pid','$barcode','','1','$TestId','$sample_id','0','0','','0','$cashDate','$cashTime','$user','2')");
			$testExists++;
		}
	...
	if($testExists)
	{
		$opdCheck	=mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `uhid_and_opdid` WHERE `patient_id`='$pid' AND `opd_id`='$barcode'"));
		if(!$opdCheck)
		{
		if(mysqli_query($link,"INSERT INTO `uhid_and_opdid`(`patient_id`, `opd_id`, `hospital_no`, `refbydoctorid`, `center_no`, `ward_id`, `wardName`, `cashMemoNo`, `branch_id`, `urgent`, `receipt_no`, `serial_no`, `reg_type`, `type`, `user`, `date`, `time`) VALUES ('$pid','$barcode','','0','','0','$ward','$bcd','1','0','$newCode','','1','2','$user','$cashDate','$cashTime')"))
		{
		}
		else
		{
			$txxt="INSERT INTO `uhid_and_opdid`(`patient_id`, `opd_id`, `hospital_no`, `refbydoctorid`, `center_no`, `ward_id`, `wardName`, `cashMemoNo`, `branch_id`, `urgent`, `receipt_no`, `serial_no`, `reg_type`, `type`, `user`, `date`, `time`) VALUES ('$pid','$barcode','','0','','0','$ward','$bcd','1','0','$newCode','','1','2','$user','$cashDate','$cashTime')";
			$file=$barcode;
			text_query_error("uhid_opd_id",$txxt,$file);
		}
		...
		{
			$txxt="SELECT * FROM `uhid_and_opdid` WHERE `patient_id`='$pid' AND `opd_id`='$barcode'";
			$file=$barcode;
			text_query_error("barcode_opd",$txxt,$file);
		}
	}
